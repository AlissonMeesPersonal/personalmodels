...
export async function POST(request:Request){
 ...
 const {data:profile,error:profileError}=await db.from('professional_profiles').select('cref,cref_state').eq('user_id',user.id).maybeSingle();
 if(profileError||!profile?.cref)return NextResponse.json({error:'Salve seu perfil antes de solicitar a análise.'},{status:400});
 const {data:review,error:reviewError}=await db.from('cref_reviews').select('status,cref,cref_state').eq('user_id',user.id).maybeSingle();
 ...
 const {error}=await db.from('cref_reviews').upsert({user_id:user.id,cref:profile.cref,cref_state:profile.cref_state,status:'pending',reviewed_at:null},{onConflict:'user_id'});
